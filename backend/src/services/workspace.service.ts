...
const NAV_ITEM_PERMISSION_REQUIREMENTS: Record<string, { all?: string[]; any?: string[] }> = {
  ...
  'driver-advances': { all: ['driver_advance_view'] },
  'driver-settlements': { all: ['driver_settlement_view'] },
  'finance-transactions': { all: ['finance_transactions_view'] },
  'finance-accounts': { all: ['finance_view'] },
  'finance-categories': { all: ['finance_view'] },
  'finance-vendors': { all: ['vendors_view'] },
  'finance-customers': { all: ['customers_view'] },
  'pod-chain': { any: ['trip_billing_view', 'finance_approve', 'driver_document_verify', 'documents_verify'] },
  'payment-closure': { any: ['payments_view', 'payments_create', 'trip_billing_view'] },
