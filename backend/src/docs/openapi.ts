    ...
    schemas: {
      ...
      Document: {
        ...
        properties: {
          ...
          linkedEntityId: { type: 'string', nullable: true },
      ...
      DocumentUploadInput: {
        type: 'object',
        required: ['title', 'documentType', 'documentCategory'],
        properties: {
          title: { type: 'string' },
          description: { type: 'string' },
          documentType: { type: 'string' },
  ...
  paths: {
    ...
    '/documents': {
      get: {
        ...
        description: 'Filter by category, type, status, entity, and more.',
        security: [{ bearerAuth: [] }],
        parameters: [
          { name: 'search', in: 'query', schema: { type: 'string' } },
          { name: 'documentType', in: 'query', schema: { type: 'string' } },
          { name: 'documentCategory', in: 'query', schema: { type: 'string', enum: ['VEHICLE', 'DRIVER', 'TRIP', 'COMPLIANCE', 'FINANCE', 'MAINTENANCE', 'REPAIR', 'VENDOR', 'CUSTOMER', 'GENERAL'] } },
          ...
          { name: 'expiringBefore', in: 'query', schema: { type: 'string', format: 'date-time' } },
          { name: 'page', in: 'query', schema: { type: 'integer', default: 1 } },
          { name: 'limit', in: 'query', schema: { type: 'integer', default: 20 } },
        ],
        responses: {
          ...
          '401': { description: 'Authentication required' },
          '403': { description: 'Missing documents_view permission' },
        },
      },
    },
    '/documents/upload': {
      post: {
        ...
        requestBody: {
          ...
          content: {
            'multipart/form-data': {
              schema: {
                type: 'object',
                required: ['file', 'title', 'documentType', 'documentCategory'],
                properties: {
                  file: { type: 'string', format: 'binary', description: 'PDF, JPEG, PNG, or WebP file (max 10MB)' },
                  title: { type: 'string' },
                  description: { type: 'string' },
                  documentType: { type: 'string' },
                  documentCategory: { type: 'string' },
        ...
        responses: {
          ...
          '401': { description: 'Authentication required' },
          '403': { description: 'Missing documents_upload permission' },
        },
      },
    },
    '/documents/{id}': {
      ...
      put: {
        ...
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string' } }],
        requestBody: {
          content: { 'application/json': { schema: { $ref: '#/components/schemas/DocumentUpdateInput' } } },
        },
        responses: {
          '200': { description: 'Document updated' },
          '401': { description: 'Authentication required' },
          '403': { description: 'Missing documents_update permission' },
          '404': { description: 'Document not found' },
        },
      },
      delete: {
        ...
        responses: {
          ...
          '401': { description: 'Authentication required' },
          '403': { description: 'Missing documents_delete permission' },
          '404': { description: 'Document not found' },
        },
      },
    },
    '/documents/{id}/view': {
      get: {
        ...
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string' } }],
        responses: {
          '200': { description: 'Document content or signed URL' },
          '401': { description: 'Authentication required' },
          '403': { description: 'Missing documents_download permission' },
          '404': { description: 'Document not found' },
        },
      },
    },
    '/documents/{id}/download': {
      get: {
        ...
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string' } }],
        responses: {
          '200': { description: 'Document download URL or content' },
          '401': { description: 'Authentication required' },
          '403': { description: 'Missing documents_download permission' },
          '404': { description: 'Document not found' },
        },
    ...
    '/documents/{id}/verify': {
      post: {
        ...
        responses: {
          '200': { description: 'Document verified' },
          '400': { description: 'Invalid status' },
          '401': { description: 'Authentication required' },
          '403': { description: 'Missing documents_verify permission' },
          '404': { description: 'Document not found' },
        },
      },
    },
    '/documents/{id}/archive': {
      post: {
        ...
        responses: {
          ...
          '401': { description: 'Authentication required' },
          '403': { description: 'Missing documents_archive permission' },
          '404': { description: 'Document not found' },
        },
      },
    },
    ...
    '/me/driver-documents': {
      get: { tags: ['Driver Portal'], summary: 'Get documents for linked driver (requires active DRIVER UserProfileLink)', security: [{ bearerAuth: [] }], parameters: [{ name: 'page', in: 'query', schema: { type: 'integer', default: 1 } }, { name: 'limit', in: 'query', schema: { type: 'integer', default: 20 } }, { name: 'documentType', in: 'query', schema: { type: 'string', enum: ['VEHICLE_RC', 'VEHICLE_INSURANCE', 'VEHICLE_PERMIT', 'VEHICLE_FITNESS', 'VEHICLE_PUC', 'ROAD_TAX', 'FASTAG', 'AIS140_GPS', 'DRIVER_LICENSE', 'DRIVER_ID_PROOF', 'TRIP_POD', 'TRIP_CHALLAN', 'TRIP_LR', 'TRIP_EWAY_BILL', 'CUSTOMER_PO', 'INVOICE', 'PAYMENT_PROOF', 'FUEL_BILL', 'EXPENSE_BILL', 'MAINTENANCE_BILL', 'REPAIR_BILL', 'VENDOR_DOCUMENT', 'CUSTOMER_DOCUMENT', 'GENERAL'] } }, { name: 'documentCategory', in: 'query', schema: { type: 'string', enum: ['VEHICLE', 'DRIVER', 'TRIP', 'COMPLIANCE', 'FINANCE', 'MAINTENANCE', 'REPAIR', 'VENDOR', 'CUSTOMER', 'GENERAL'] } }], responses: { '200': { description: 'Paginated documents for linked driver' } } },
    },
    '/me/driver-fuel': {
