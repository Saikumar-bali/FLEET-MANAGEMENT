...
async function main() {
  ...
  const mismatch = await call('/fuel', admin, 'POST', { entryMode: 'FULL_DETAILS', vehicleId: other.data.data.id, tripId: trip.data.data.id, fuelDate: new Date().toISOString(), fuelType: 'DIESEL', quantityLiters: 10, pricePerLiter: 100 }); check('fuel trip vehicle mismatch rejected', mismatch, 400);
  const fuel = await call('/fuel', admin, 'POST', { entryMode: 'FULL_DETAILS', vehicleId: vehicle.data.data.id, tripId: trip.data.data.id, fuelDate: new Date().toISOString(), fuelType: 'DIESEL', quantityLiters: 10, pricePerLiter: 100, stationName: 'TEST-E2E Station' }); check('create fuel', fuel, 201);
  ...
  const rejectedFuel = await call('/fuel', admin, 'POST', { entryMode: 'FULL_DETAILS', vehicleId: vehicle.data.data.id, fuelDate: new Date().toISOString(), fuelType: 'DIESEL', quantityLiters: 5, pricePerLiter: 100 }); await call(`/fuel/${rejectedFuel.data.data.id}/submit`, admin, 'POST', {}); check('reject fuel', await call(`/fuel/${rejectedFuel.data.data.id}/reject`, admin, 'POST', {}), 200);
