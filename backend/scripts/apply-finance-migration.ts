/**
 * Apply incremental finance India-native fields migration to existing database.
 * This script adds missing columns to customers, vendors, trip_billings, and payment_records.
 * Safe to run multiple times (uses IF NOT EXISTS).
 *
 * Usage: npx tsx scripts/apply-finance-migration.ts
 */
...
const ALTER_STATEMENTS = [
  // --- customers: add India-native columns ---
  ...
  // Add unique index on customer_code if not exists
  ...
  // --- vendors: add India-native columns ---
  ...
  // Add unique index on vendor_code if not exists
  ...
  // --- trip_billings: add India-native columns and FKs ---
  ...
  // Add FK constraints for trip_billings if not exists
  ...
  // --- payment_records: add India-native columns ---
  ...
  // Add unique index on payment_number
...
async function main() {
  console.log('Applying incremental finance migration to Neon database...');
  let applied = 0;
  let skipped = 0;
  ...
  for (const sql of ALTER_STATEMENTS) {
    try {
      ...
      applied++;
      const short = sql.substring(0, 80);
      console.log(`  ✓ ${short}...`);
    } catch (err: any) {
      skipped++;
      const short = sql.substring(0, 80);
      console.log(`  ⚠ ${short}... (${err.message?.substring(0, 80)})`);
    }
  }

  console.log(`\nDone: ${applied} applied, ${skipped} skipped/warned.`);
