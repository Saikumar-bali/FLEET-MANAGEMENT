  ...
  useEffect(() => {
    void (async () => {
      ...
      try {
        ...
        setError(`Failed to load ${kind} records.`);
      } finally {
        setLoading(false);
      }
    })();
  }, [auth.accessToken, kind, search, status]);

  if (loading) return <LoadingState message={`Loading ${kind} records...`} />;
      ...
      {items.length ? (
        <div className="card table-card">
          <DataTable
            ...
            data={items}
            keyExtractor={(x) => x.id}
            onRowClick={(x) => navigate(`/${basePath}/${x.id}`)}
          />
        </div>
      ) : (
        <EmptyState message={`No ${kind} records found.`} />
      )}
    </section>
  );
}
