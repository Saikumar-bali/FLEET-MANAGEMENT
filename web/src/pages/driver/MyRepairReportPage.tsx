...
export function MyRepairReportPage() {
  const auth = useAuth();
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/drivers/me/repair-reports`, {
        method: 'POST',
  ...
  return (
    <PageShell>
      <div style={{ maxWidth: '500px' }}>
        <h2 style={{ margin: '0 0 var(--space-4)' }}>Report Repair Issue</h2>
        ...
        <form onSubmit={handleSubmit} className="stack-form">
          ...
          <label><span className="field-label">Description *</span>
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required rows={4} placeholder="Describe the repair issue..." />
          </label>
          <div className="button-row">
            <button type="submit" className="primary-button" disabled={isSaving}>{isSaving ? 'Submitting...' : 'Submit Report'}</button>
