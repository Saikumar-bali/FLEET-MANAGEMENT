...
function severityClass(severity?: string) {
  ...
  if (normalized.includes('error') || normalized.includes('critical') || normalized.includes('danger')) return 'alert-tone-danger';
  if (normalized.includes('warn')) return 'alert-tone-warning';
  if (normalized.includes('success')) return 'alert-tone-success';
...
export function AlertsPage() {
  ...
  const handleMarkOneRead = async (item: NotificationItem) => {
    ...
    await markRead(auth.accessToken, item.id);
  ...
  const handleMarkAllRead = async () => {
    ...
    await markAllRead(auth.accessToken);
    ...
    <section className="alerts-page page-content">
      <div className="alerts-hero card">
        ...
        <div className="alerts-actions">
          <button type="button" className="secondary-button" onClick={() => void load(true)} disabled={refreshing}>
            {refreshing ? 'Refreshing...' : 'Refresh'}
          </button>
          <button type="button" className="primary-button" onClick={() => void handleMarkAllRead()} disabled={unreadItems.length === 0}>
            Mark all read
          </button>
        </div>
      </div>
      ...
      <div className="alerts-summary-grid">
        <div className="alerts-summary-card card">
          <span>Total alerts</span>
          <strong>{items.length}</strong>
        </div>
        <div className="alerts-summary-card card alerts-summary-unread">
          <span>Unread</span>
                ...
                <article key={item.id} className={`alert-card ${unread ? 'alert-card-unread' : ''}`}>
                  ...
                  <div className="alert-main">
                    ...
                    <div className="alert-meta-row">
                      ...
                      {unread && <span className="alert-new-pill">New</span>}
                    </div>
                  </div>
                  <div className="alert-card-actions">
                    {item.actionUrl && (
                      <button type="button" className="secondary-button" onClick={() => { window.location.href = item.actionUrl || '/alerts'; }}>
                        Open
                      </button>
                    )}
                    {unread && (
                      <button type="button" className="ghost-button" onClick={() => void handleMarkOneRead(item)}>
                        Mark read
                      </button>
                    )}
                  </div>
