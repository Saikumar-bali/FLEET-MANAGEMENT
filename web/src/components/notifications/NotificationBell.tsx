...
export function NotificationBell() {
  ...
  return (
    <div style={{ position: 'relative' }}>
      <button type="button" className="btn-secondary" onClick={() => { setOpen(!open); void refresh(); }}>
        Alerts {count > 0 ? `(${count})` : ''}
      </button>
      {open && (
        <div style={{ position: 'absolute', right: 0, top: 40, width: 300, padding: 12, border: '1px solid var(--color-border)', borderRadius: 12, background: 'var(--color-surface)', zIndex: 50 }}>
