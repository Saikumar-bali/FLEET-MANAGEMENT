...
function ItemIcon({ label }: { label: string }) {
  ...
  const icons: Record<string, JSX.Element> = {
    ...
    Playground: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="5 3 19 12 5 21 5 3" /></svg>,
...
export function Sidebar({ isOpen, onClose, isCollapsed, onToggleCollapse, onOpenSettings, onOpenAccount }: SidebarProps) {
  ...
  const visibleSections = sidebarSections.map((section) => ({
    ...section,
    items: section.items.filter((item) => auth.hasAnyPermission(item.permissionKeys)),
      ...
      <aside className={`sidebar${isOpen ? ' sidebar-open' : ''}`}>
        ...
        <div className="sidebar-footer">
          ...
          <div className="sidebar-icon-bar">
            ...
            <button type="button" className="sidebar-icon-bar-button" title="API key">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
